UnitList = {

          T3LandAA = {
              UEF = {'kel0305', 50}, -- Bastion (Mobile Combined Defense Platform) 
          },
          T3ArmoredAssault = {
              Cybran = {'krl0301', 50}, -- Jody (Main Battle Tank) 
              Aeon = {'kal0301', 50}, -- Jarvis (Main Battle Tank) 
              Seraphim = {'ksl0301', 50}, -- Juun tzu (Main Battle Tank) 
          },
		  T1LandDFTank = {
		  UEF = {'kel0101', 60}, -- M551 Sheridan (Heavy Tank) 
		  },
		  T2LandDFTank = {
			  UEF = {'kel0201', 70}, -- Paddur (Mobile APC) 
			  Seraphim = {'ksl0201', 50}, -- Name: Aranuhn (Mobile APC) 
		  },
          T4LandExperimental1 = {
              UEF = {'kel0405', 50}, -- King Kriptor (Experimental Assault Bot) 
              Cybran = {'krl0401', 50}, -- Diablo (Experimental Mobile Tank) 
              Aeon = {'kal0401', 50}, -- Slim boy (Experimental Mobile Tank) 
              Seraphim = {'ksl0401', 50}, -- hae fiu (Heavy Rocket Tank) 
          },
          T1Gunship = {
              UEF = {'kea0101', 50}, -- Blackfish (Gunship) 
          },
          T2Artillery = {
              UEF = {'keb2202', 50}, -- Dwarf star (Mortar Installation) 
          },
          T3GroundDefense = {
              UEF = {'keb2301', 20}, -- Naval Cannon (Heavy Naval Artillery Bunker) 
              Cybran = {'krb2301', 20}, -- Naval Cannon (Heavy Naval Artillery Bunker) 
              Aeon = {'kab2301', 20}, -- Elea (Heavy Naval Artillery Bunker) 
              Seraphim = {'ksb2301', 20}, -- Haratham (Heavy Naval Artillery Bunker) 
          },
          T3LandArtillery = {
              UEF = {'kel0308', 50}, -- Nuker (Nuclear Cannon) 
              Aeon = {'kal0304', 50}, -- Insanity (MLRS) 
          },
          T4AirExperimental1 = {
              UEF = {'kea0401', 50}, -- Airboy (Experimental Gunship) 
              Aeon = {'kaa0401', 50}, -- HMP Restorator (Experimental Gunship) 
          },
          T3AADefense = {
              UEF = {'keb2305', 50}, -- Kashtan (Close-in-Weapon-System) 
          },
          T3AirBomber = {
              UEF = {'kea0301', 20}, -- B-52 Stratofortress (Nuclear Bomber) 
          },
          T2SeaCruiser = {
              UEF = {'kes0201', 50}, -- Beirut Class (Tech 2 Early Battleship) 
          },
          T2GroundDefense = {
              UEF = {'keb2201', 50}, -- Pack Gun (Point Defense) 
              Seraphim = {'ksb2201', 50}, -- Umuushal (Point Defense) 
          },
          T3SeaBattleship = {
              UEF = {'kes0302', 70}, -- Iwojima Class (Heavy Destroyer) 
              Cybran = {'krs0302', 70}, -- Sadam Class (Heavy Destroyer) 
              Aeon = {'kas0302', 70}, -- Infiltrator Class (Heavy Destroyer) 
              Seraphim = {'kss0301', 70}, -- Uan shal voh (Heavy Destroyer) 
          },
          T3Battlecruiser = {
              UEF = {'kes0401', 90}, -- Guada Class (Heavy Cruiser)
          },
	}